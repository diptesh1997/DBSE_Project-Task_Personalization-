def main():
    ...
    flag=0
    for i in (row[0]):
      ans3_11 = D_L_model.D_L_model(str(i))
      if(ans3_11!=ans3):
       flag=1
      ans3_1.append(ans3_11)
    #print(ans3_1)
    #print(flag)
    import Edge_cases
    i1=0
    if (flag==1):
          print('This is a edge case scenario where each proper noun have differnet semantical meaning, edge case scanario was not integrated with the main system as its slowing the entire system down')
          print('kindly run edge case.py file with original questions and extracted words')
          print('The extracted words:',row[0])
          print('The original question is:',ans1)
          return
          #include a loop
          #ans = Edge_cases.Edge_cases(str(ans1),row[0])
          #print(ans)
          #i1=i1+1
          #return(ans)
    ...
    #def replace():
    answ = []
    def replace(t): #It can handle upto 3 proper Nouns in a sentences # if the proper nouns are of same words it will be replaced by same words only
        #answ = []
